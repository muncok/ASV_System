...
from prototypical_loss import prototypical_loss as p_loss
from verification_loss import verification_loss as v_loss
from verification_loss import verification_score as eer_score
from verification_loss import verification_optimal_score as opt_score
...
def init_proto_loaders(opt):
    ...
    '''
    train_dataset, val_dataset = SpeechDataset.read_train_manifest(opt)
...
def sv_score(opt, val_dataloader, model):
    ...
    sleep(0.05)
    # print("input was splited to {}".format(len(model_outputs)))
...
def sv_optimal_score(opt, val_dataloader, model, filter_types, lda=None):
    val_iter = iter(val_dataloader)
    nb_splicing = opt.input_length // opt.splice_length
...
def main():
    ...
    options.input_length = int(16000*2.0)
    ...
    if options.mode == "train":
        ...
        lda = pickle.load(open("lda_voxc.pkl", "rb"))
        # sv_optimal_score(options, val_dataloader, model, ["full", "random", "diff"])
        sv_optimal_score(options, val_dataloader, model, ["full", "random", "diff", "std"], lda)
        ...
        train_X, train_y = embedings[random_idx[:n_samples-1000]], labels[random_idx[:n_samples-1000]]
        test_X, test_y = embedings[random_idx[-1000:]], labels[random_idx[-1000:]]
